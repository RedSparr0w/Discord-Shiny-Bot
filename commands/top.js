...
module.exports = {
  ...
  execute     : async (msg, args) => {
    ...
    const output = [`__***Top ${results.length} ${tables[table]}:***__`, ...results.map((res, place) => `**#${place + 1}** _\`(${res.points.toLocaleString('en-NZ')} ${table})\`_ ${msg.guild.members.cache.get(res.user) || 'Inactive Member'}`)];
    if (output.join('\n').length >= 2000)
